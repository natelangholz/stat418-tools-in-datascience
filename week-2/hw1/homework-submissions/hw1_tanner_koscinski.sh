...
echo
# Look at some browser names:
# curl -s http://users.csc.tntech.edu/~elbrown/access_log.bz2 | bunzip2 - | cut -d ' ' -f 12- | grep '^$' -v | grep Mozilla -v | sort | uniq -c | sort -nr | head -n 50
n=($(curl -s http://users.csc.tntech.edu/~elbrown/access_log.bz2 | bunzip2 - | cut -d ' ' -f 12- | wc -l))
# Real:
...
echo The traffic is mostly real.
